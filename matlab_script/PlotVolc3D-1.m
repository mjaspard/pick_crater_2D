...
ax(2)=subplot(2,2,2);
imagesc(distotprojforinterp/slra,y/azim,MATDIST) %%% PB !!
colormap gray
...
caxis([cmean-cmean*80/100 cmean+cmean*150/100])
 hold on
 plot(caldera_proj(:,1)/sind(incid)/slra,caldera_proj(:,2)/azim,'b')
 plot(P2_proj(:,1)/sind(incid)/slra,P2_proj(:,2)/azim,'c')
 plot(crat_proj(:,1)/sind(incid)/slra,crat_proj(:,2)/azim,'r')
 plot(bot_proj(:,1)/sind(incid)/slra,bot_proj(:,2)/azim,'m')
 plot(interm_proj(:,1)/sind(incid)/slra,interm_proj(:,2)/azim,'Color',[1 0.5 0])
 plot(base_proj(:,1)/sind(incid)/slra,base_proj(:,2)/azim,'k')
 %plot(proj(:,1)/sind(incid)/slra,proj(:,2)/azim,'g')
 %plot(proj2(:,1)/sind(incid)/slra,proj2(:,2)/azim,'k')
xlabel('Range (Pixels)')
ylabel('Azimuth (Pixels)')
axis image
axis normal
